...
/*void (vector<Client> Bdd_client){
    int id;
    cout<<"Entrez l'id du client que vous voulez rechercher"<<endl;
    cin>>id;
    for(int i=0;i<Bdd_client.size();i++){
        if(Bdd_client[i].get_id()==id){
            Bdd_client[i].afficher();
        }
    }
}*/
/*
int chercher_client(vector<Client> Bdd_client,string name,string surname){
    int id=1;
    for(int i=0;i<Bdd_client.size();i++){
        if(Bdd_client[i].get_name()==name && Bdd_client[i].get_surname()==surname){
            id=Bdd_client[i].get_id();
            cout<<"id: "<<id<<endl;
            Bdd_client[i].affiche();
