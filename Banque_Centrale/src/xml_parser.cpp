...
Client::Client(int id,int id_agence,string name,string surname,int age,string password,int idcompte_courant,unsigned int solde_courant,int idcompte_epargne1,unsigned int solde_epargne1,int idcompte_epargne2,unsigned int solde_epargne2){
    this->id=id;
...
void Client::set_id_agence(int id_agence){
        this->id_agence=id_agence;
}
...
vector<Client> reader(){
    ...
    for(xml_node<> *node = dims->first_node("client"); node; node = node->next_sibling()){
        ...
        id_agence=atoi(node->first_node("id_agence")->value());
        ...
        Bdd_client.push_back(Client(id,id_agence,name,surname,age,password,idcompte_courant,solde_courant,idcompte_epargne1,solde_epargne1,idcompte_epargne2,solde_epargne2));
...
void writer(vector<Client> Bdd_client){
    ...
    for(int i=0;i<Bdd_client.size();i++){
        ...
        file<<"<id_agence>"<<Bdd_client[i].get_id_agence()<<"</id_agence>"<<endl;
